...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto transition-colors duration-300">
      ...
      <Card className="p-12 border-2 border-dashed text-center bg-gray-50/50 dark:bg-zinc-900/50 hover:border-blue-400 transition-all cursor-pointer rounded-[2rem]" onClick={() => document.getElementById('fileIn')?.click()}>
        ...
        <h3 className="font-bold text-gray-700 dark:text-zinc-200 text-lg">{fileName || "Load External Dataset"}</h3>
        <p className="text-xs text-gray-400 mt-1">UTF-8 CSV Standard Protocol</p>
      </Card>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {operationsList.map((op) => (
          <Card key={op.id} onClick={() => setSelectedOps(prev => prev.includes(op.id) ? prev.filter(i => i !== op.id) : [...prev, op.id])}
            className={`p-6 cursor-pointer border-2 transition-all group ${selectedOps.includes(op.id) ? "border-blue-600 bg-blue-50/50 dark:bg-blue-900/40" : "border-gray-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-blue-200"}`}>
      ...
      <Button onClick={handleRunAll} disabled={isLoading} className="w-full py-8 text-xl bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition-all active:scale-[0.98]">
        {isLoading ? "Synchronizing Engines..." : "Initialize Parallel Analysis"}
      </Button>
      ...
      <div className="pt-10 border-t mt-12 bg-gray-50/30 dark:bg-zinc-900/30 p-6 rounded-[2rem]">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-8">
            ...
            <Button size="lg" onClick={handleSearch} className="bg-blue-600 hover:bg-blue-700 text-white rounded-2xl px-10 h-14 font-bold active:scale-95 shadow-md">Search Database</Button>
        </div>
